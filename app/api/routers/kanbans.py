...
async def get_items_from_kanban(
    kanban_name: str = Path(..., title="kanban Name"),
...
async def create_new_kanban(
    ...
    return {"message": f"Successfully Create Kanban: {kanban_name}", "response": res}


@router.delete("/{kanban_name}")
async def delete_kanban(kanban_name: str = Path(..., title="Kanban Name")):
    """ """
    es = get_es_client()
    res = es.indices.delete(index=kanban_name)

    return {"message": f"Successfully Delete Kanban: {kanban_name}", "response": res}
...
async def create_or_update_item_from_kanban(
    ...
    res = es.index(index=kanban_name, doc_type="_doc", id=item_id, body=data)
